...
function getAddressParts(miner) {
    var address = '';
    var workerName = '';
    var miner2 = String(miner);
    var nameOffset = miner2.indexOf('+');	
    if (nameOffset != -1) {
        address = miner2.substr(0, nameOffset);
        workerName = miner2.substr(nameOffset + 1);
    } else {
	 var nameOffset2 = miner2.indexOf(',');
	 if (nameOffset2 != -1) {
		address = miner2.substr(0, nameOffset2);

		var nameOffset3 = address.indexOf('.');
		if (nameOffset3 != -1) {
			address = address.substr(0, nameOffset3);
		}
	 workerName = miner2.substr(nameOffset2 + 1);
   	} else {
        address = miner;
    }}
...
function broadcastLiveStats(){
    ...
    redisClient.multi(redisCommands).exec(function(error, replies){
        ...
        for (var i = 0; i < addresses.length; i++){
            ...
            var workersData = [];
            for (var ii=0; ii<replies[offset + 2].length; ii++) {
                var key = replies[offset + 2][ii];
...
function handlePortStats(urlParts, response){
    response.writeHead(200, {
        'Access-Control-Allow-Origin': '*',
        'Cache-Control': 'no-cache',
        'Content-Type': 'application/json',
    ...
    response.write('\n');
    var port = urlParts.query.port;

    if (urlParts.query.longpoll === 'true'){
        redisClient.exists(config.coin + ':ports:', port, function(error, result){
            if (!result){
                response.end(JSON.stringify({error: 'Not found'}));
                return;
            }
            //portConnections[port] = response;
            response.on('finish', function(){
            //delete portConnections[port];
            })
        });
    }
    else{
        redisClient.hget(config.coin + ':ports:' + port, 'miners', function(error, result) {
        if (error) {
			log('error', logSystem, 'Unable get port info!');
			return;
        }
		//log('info', logSystem, 'Getting port ' +port+' info: Miners: ' + result +'!');
        ...
        function(workerKeys, callback) {
            ...
            redisClient.multi(redisCommands).exec(function(error, redisData) {
                ...
                for(var i in redisData) {
					...
					data.push({ miner: address.substring(0,7)+'****'+address.substring(89), hashrate: minersHashrate[address] ? minersHashrate[address] : 0 });
                }
                callback(null, data);
            });
        }
...
function handleServerRequest(request, response) {
    ...
    switch(urlParts.pathname){
        ...
        case '/stats_address':
            ...
            break;
		case '/stats_port':
				handlePortStats(urlParts, response);
				break;			
        case '/get_payments':
