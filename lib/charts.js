...
function storeCollectedValue(chartName, value, settings) {
    ...
    getChartDataFromRedis(chartName, function(sets) {
        ...
        if(getStatsRedisKey(chartName).search(config.coin + ":charts:hashrate") >=0){
         redisClient.set(getStatsRedisKey(chartName), JSON.stringify(sets), 'EX', (settings.maximumPeriod * 7));
        }else{
...
function getCoinPrice(callback) {
    ...
    if (source == 'cryptonator'){
        ...
        }, '/api/ticker/' + config.symbol.toLowerCase() + '-' + currency.toLowerCase());
    }

    // TradeOgre
    else if (source == 'tradeogre'){
        apiInterfaces.jsonHttpRequest('tradeogre.com', 443, '', function(error, response) {
            if (error || response.error) callback(response.message ? response.message : error, null);
            else {
                var priceBTC = +response.price || null;
                if (priceBTC) {
                    apiInterfaces.jsonHttpRequest('api.cryptonator.com', 443, '', function(error, response) {
                        var priceCurrency = response.success ? +response.ticker.price : null;
                        var price = priceCurrency ? priceBTC*priceCurrency : null;
                        callback(response.error ? response.error : error, +price);
                    }, '/api/ticker/btc-' + currency.toLowerCase());
                }
                else callback(null, null);
            }
        }, '/api/v1/ticker/BTC-' + config.symbol.toUpperCase());
    }
}
