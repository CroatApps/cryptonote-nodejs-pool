...
exports.sendEmail = function(email, subject, template, variables) {
    //if (!config.email || !config.email.enabled || !!config.email.templates || !email) return ;
    if (!config.email.enabled || !config.email.templates || !email) 
    {
    log('info', logSystem, 'Error starting E-Mail system!');        
    return ;
    }
