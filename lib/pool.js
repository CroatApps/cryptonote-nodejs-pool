...
var moneroVariant = config.moneroVariant || isMonero;
...
setInterval(function(){
    ...
    for (var minerId in connectedMiners){
        ...
        if (now - miner.lastBeat > timeout){
			...
			redisClient.hincrby([config.coin + ':ports:'+miner.port, 'miners', '-1']);			
...
function newConnectedWorker(miner){
    log('info', logSystem, 'Miner connected %s@%s:%s', [miner.login, miner.ip, miner.port]);
	redisClient.hincrby([config.coin + ':ports:'+miner.port, 'miners', '1']);	
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    redisCommands.push(['expire', config.coin + ':workers:' + miner.login, config.userHistory.maxWalletHistory * 86400]);
    redisCommands.push(['expire', config.coin + ':payments:' + miner.login, config.userHistory.maxWalletHistory * 86400]);
    ...
    if (miner.workerName) {
        ...
        redisCommands.push(['expire', config.coin + ':unique_workers:' + miner.login + '+' + miner.workerName, (86400 * 7)]);
    }
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        var cn_variant = moneroVariant && convertedBlob[0] >= 7 ? convertedBlob[0] - 6 : 0;
    ...
    async.each(config.poolServer.ports, function(portData, cback){
        ...
        if (portData.ssl) {
            ...
            tls.createServer(options, socketResponder).listen(portData.port, function (error, result) {
                ...
                log('info', logSystem, 'Clear values for SSL port %d in redis.', [portData.port]);				
            ...
            net.createServer(socketResponder).listen(portData.port, function (error, result) {
                ...
                log('info', logSystem, 'Clear values for port %d in redis.', [portData.port]);				
				redisClient.del([config.coin + ':ports:'+portData.port]);		
				redisClient.hset([config.coin + ':ports:'+portData.port, 'port', portData.port ]);				
