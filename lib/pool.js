...
function Miner(id, login, pass, ip, workerName, email, startingDiff, noRetarget, pushMessage){
    this.id = id;
    this.login = login;
    this.pass = pass;
    this.ip = ip;
...
function newConnectedWorker(miner){
    log('info', logSystem, 'Miner connected %s@%s', [miner.login, miner.ip]);
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            miner = new Miner(minerId, login, pass, ip, workerName, email, difficulty, noRetarget, pushMessage);
